...
export const webAssignAssetToUserHandler = async (ctx: MutationCtx, args: Args) => {
  ...
  if (userToAssign.role !== 'manager') {
    const userProperty = await ctx.db
      .query('properties')
      .withIndex('by_manager', q => q.eq('managerId', userToAssign._id))
  ...
  await ctx.db.insert('assetHistory', {
    ...
    notes: args.notes || `Asset assigned to ${userToAssign.firstName} ${userToAssign.lastName}`,
    timestamp: now,
  ...
  if (!updatedAsset) throw new Error('Failed to assign asset');
