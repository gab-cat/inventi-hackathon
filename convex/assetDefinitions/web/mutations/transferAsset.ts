...
export const webTransferAssetArgs = {
  assetId: v.id('assets'),
  toUserId: v.id('users'),
...
export const webTransferAssetReturns = v.object({
  _id: v.id('assets'),
...
export const webTransferAssetHandler = async (ctx: MutationCtx, args: Args) => {
  ...
  const now = Date.now();
  const newLocation = args.toLocation || asset.location;
  ...
  await ctx.db.insert('assetHistory', {
    ...
    notes: args.notes || `Asset transferred to ${toUser.firstName} ${toUser.lastName}`,
    timestamp: now,
  ...
  if (!updatedAsset) throw new Error('Failed to transfer asset');
