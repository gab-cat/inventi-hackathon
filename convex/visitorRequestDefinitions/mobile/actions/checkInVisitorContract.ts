...
export const mobileCheckInVisitorContractArgs = v.object({
  piiHash: v.string(), // Will be converted to bytes32
});

export const mobileCheckInVisitorContractReturns = v.object({
  success: v.boolean(),
  transactionHash: v.optional(v.string()),
  message: v.optional(v.string()),
});

export const mobileCheckInVisitorContractHandler = async (ctx: ActionCtx, args: { piiHash: string }) => {
  try {
    ...
    // TODO: Update database with visitor check-in status
    // This would require creating an internal mutation to update visitor status
    console.log(`Visitor checked in for piiHash: ${args.piiHash}, tx: ${hash}, block: ${receipt.blockNumber}`);
