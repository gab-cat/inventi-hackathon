...
// A safe wrapper that uses Animated.View on native and plain View on web,
// stripping unsupported props to avoid warnings.
function NativeOnlyAnimatedView({ entering, exiting, ...props }: any) {
  if (Platform.OS === 'web') {
    return <View {...props} />;
  }
      ...
      <FullWindowOverlay>
        <SelectPrimitive.Overlay
          ...
          style={Platform.select({
            native: { zIndex: 9999, elevation: 9999 },
          })}
            ...
            <TextClassContext.Provider value='text-popover-foreground'>
              <NativeOnlyAnimatedView entering={FadeIn.delay(50)} exiting={FadeOut.duration(150)}>
                <SelectPrimitive.Content
                  className={cn(
                    'bg-popover border-border relative z-50 min-w-[8rem] max-w-[calc(100%-2rem)] rounded-md border shadow-md shadow-black/5',
                    Platform.select({
                      web: cn(
