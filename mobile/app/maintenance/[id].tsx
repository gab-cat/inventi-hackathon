    ...
    <ThemedView style={{ flex: 1 }} className='bg-slate-50'>
      ...
      {req ? (
        <ScrollView contentContainerStyle={{ padding: 12, paddingTop: 6 }} showsVerticalScrollIndicator={false}>
          ...
          <View className='bg-blue-800 rounded-xl p-4 mb-3'>
            <View className='flex-row justify-between items-start mb-3'>
              <View className='flex-1'>
                <Text className='text-white text-2xl font-bold leading-tight mb-2'>{req.title}</Text>
                <Text className='text-blue-100 text-sm leading-relaxed'>{req.description}</Text>
              </View>
              <View className='ml-3'>
                <View
                  className={`
                ...
                `}
                >
                  <Icon as={getStatusIcon(req.status)} size={24} className='text-white' />
                </View>
              </View>
            </View>
            ...
            <View className='flex-row justify-between items-center'>
              ...
              `}
              >
                <Text className={`text-white text-xs font-bold uppercase tracking-wide`}>{req.priority}</Text>
              </View>
            </View>
          </View>
          ...
          <View className='bg-white rounded-lg p-3 border border-gray-100 mb-3'>
            <View className='flex-row items-center gap-2 mb-3'>
              ...
              <Text className='text-base font-bold text-gray-900'>Timeline</Text>
            </View>
              ...
              {req.assignedAt && (
                <View className='flex-row items-center gap-3'>
                  ...
                  <View className='flex-1'>
                    ...
                    <Text className='text-xs text-gray-500'>
                      {new Date(req.assignedAt).toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric',
                      })}
          ...
          <View className='bg-white rounded-lg p-3 border border-gray-100'>
            <View className='flex-row items-center gap-2 mb-3'>
              ...
              <Text className='text-base font-bold text-gray-900'>Updates</Text>
            ...
            {updates && updates.length > 0 ? (
              <View className='space-y-2'>
                {updates.map((update, index) => (
                  <View key={update._id} className='flex-row gap-3'>
                    ...
                    <View className='flex-1'>
                      ...
                      {update.description && (
                        <Text className='text-sm text-gray-600 leading-relaxed'>{update.description}</Text>
                      )}
                    </View>
                  </View>
                ))}
